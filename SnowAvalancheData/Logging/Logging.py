...
"""This module setups the logging for SnowAvalancheData.
...
import SnowAvalancheData.Config.ConfigInstall as ConfigInstall

####################################################################################################

def setup_logging(
        application_name='SnowAvalancheData',
    ...
    Alternatively we can set the logging level using the environment variable 'SnowAvalancheDataLogLevel' or
    using *logging_level*, level can be a integer or a string.  The logging level is set by
    precedence using :

    * `logging_level` parameter if not `None`
    * 'SnowAvalancheDataLogLevel' environment variable set to: CRITICAL, ERROR, WARNING, INFO, DEBUG
    * else :file:`logging.yml` file settings
    ...
    if logging_level is None and 'SnowAvalancheDataLogLevel' in os.environ:
        level_name = os.environ['SnowAvalancheDataLogLevel']
        try:
            logging_level = getattr(logging, level_name.upper())
        except AttributeError:
            sys.exit(f'SnowAvalancheDataLogLevel environment variable is set to an invalid logging level "{level_name}"')
    if logging_level:
